...
export default function StatisticQuantityOrder() {
  ...
  useEffect(() => {
    ...
    fetch(`${ApiOrder.ADMIN_STATISTIC_QUANTITY}?year=${year}&month=${month}`, {
      method: "GET",
      headers: {
  ...
  return (
    <div className="rounded-lg dark:border-gray-600 h-32 md:h-32">
      <div className="items-center justify-between p-4 bg-white border border-gray-200 rounded-lg shadow-sm sm:flex dark:border-gray-700 sm:p-6 dark:bg-gray-800">
        <div className="w-ful">
          <h3 className="text-base font-normal text-gray-500 dark:text-gray-400">Orders</h3>
          ...
          <p className="flex items-center text-base font-normal text-gray-500 dark:text-gray-400">
            {
              ...
              <span className="flex items-center mr-1.5 text-sm text-green-500 dark:text-green-400">
                ...
                {quantity ? Number(quantity.quantityCurrent - quantity.quantityPrevious).toLocaleString('vi') : 0}
              </span> :
              quantity && quantity.quantityCurrent - quantity.quantityPrevious < 0 ?
