...
public class ChooseHairColorFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.fragment_choose_hair_color, container, false);
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        AppUtils.saveDataToSharedPreferences("preFragment", "ChooseHairColorFragment", getContext());
        ...
        btnOk.setOnClickListener(v -> {
            ...
            returnBundle.putString("url", colorImageUrl);
            returnBundle.putString("color", color);
            returnBundle.putString("colorCode", colorCode);
            bundle.putBundle("colorImage", returnBundle);
            ChooseHairColorFragment.this.getParentFragmentManager().setFragmentResult("colorImage", bundle);
            ChooseHairColorFragment.this.getParentFragmentManager().popBackStack();
        });

        btnBack.setOnClickListener(v -> {
            MainActivity mainActivity = (MainActivity) ChooseHairColorFragment.this.getContext();
        ...
        colorImageAdapter.setItemClickListener((v, position) -> {
            View previousImageItemView = AppUtils.getViewFromRecyclerView(rcvListColorImage, currentPosition);
            View currentImageItemView = AppUtils.getViewFromRecyclerView(rcvListColorImage, position);
            ...
            colorImageUrl = listColorImageResponse.get(position).get("url").toString();
    ...
    private void getFilterQuery() {
        ...
        for (int i = 0; i < listHairColorResponse.size(); i++) {
            ...
            if (radioButton.getId() == colorRadioGroup.getCheckedRadioButtonId()) {
                ...
                colorCode = listHairColorResponse.get(i).get("colorCode").toString();
                break;
            }
        }
    }
                    ...
                    public void onResponse(Call<Map<String, Object>> call, Response<Map<String, Object>> response) {
                        ...
                        try {
                            ...
                            if (!response.isSuccessful()) {
                                ...
                                if (statusCode == 500) {
                                    AppUtils.show((Activity) ChooseHairColorFragment.this.getContext(), R.id.errorConstrainLayout, R.layout.error_dialog, R.id.errorDesc, R.id.errorDone, "Internal server error!");
                                    return;
                                }
                                if (statusCode == 404) {
                                    AppUtils.show((Activity) ChooseHairColorFragment.this.getContext(), R.id.errorConstrainLayout, R.layout.error_dialog, R.id.errorDesc, R.id.errorDone, "Resources not found!");
                                    return;
                                }
                                ...
                                AppUtils.show((Activity) ChooseHairColorFragment.this.getContext(), R.id.errorConstrainLayout, R.layout.error_dialog, R.id.errorDesc, R.id.errorDone, message);
                                return;
                            }
                            ...
                            customProgressDialog.close();
                            AppUtils.show((Activity) ChooseHairColorFragment.this.getContext(), R.id.errorConstrainLayout, R.layout.error_dialog, R.id.errorDesc, R.id.errorDone, "Đã có lỗi xảy ra vui lòng thử lại sau!");
                            e.printStackTrace();
                        }
                    }
    ...
    private void renderColorRadioButtons() {
        ...
        for (Map<String, Object> color : listHairColorResponse) {
            RadioButton radioButton = new RadioButton(ChooseHairColorFragment.this.getContext());
        ...
        if (colorRadioGroup.getChildCount() > 0) {
            ...
            colorCode = listHairColorResponse.get(0).get("colorCode").toString();
        }
    }
